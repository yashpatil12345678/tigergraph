...
import { dataFilesStatus, getInvestigationEvidence, normalize } from '@/lib/data/investigation-data'
import { checkTigerGraph, executeAutoAction, queryInvestigationGraph, writeCaseToTigerGraph } from '@/lib/tigergraph/client'
...
export async function runInvestigationAgent(input: { case_id: string; customer_id?: string; card_id?: string; flagged_txn_id?: number }) : Promise<InvestigationResult> {
  ...
  if (readiness.status !== 'DATA_READY') throw new Error(`DATA_INCOMPLETE: required official datasets are missing (${readiness.files.filter((item) => !item.available).map((item) => item.file).join(', ')})`)
  ...
  final = applyPolicy({ probability: assessment.probability, pattern: assessment.pattern, exposure, sharedOrigin: assessment.independent.has('graph'), conflictingEvidence: insufficient })
  for (const action of final) { if (action.route === 'AUTO') await executeAutoAction(action.action, action.reason) }
  ...
  const writeResult = final.some((item) => item.action === 'CREATE_CASE') ? await writeCaseToTigerGraph({ caseId: selected.case_id, customerId: selected.customer_id, cardId: selected.card_id, transactionId: selected.flagged_txn_id, outcome: assessment.probability >= 0.85 ? 'fraud' : 'uncertain', exposure }) : { written: false, error: 'Case creation was not selected by policy.' }
  tool(context, 'write_case', { case_id: selected.case_id }, writeResult.written ? 'success' : 'unavailable', writeResult.written ? 'TigerGraph case write succeeded.' : writeResult.error ?? 'Case write did not succeed.')
  const status = final.some((item) => item.route !== 'AUTO') ? 'escalated' : 'open'
  const verdict = insufficient ? 'uncertain' : assessment.probability >= 0.85 ? 'fraud' : assessment.probability <= 0.15 ? 'legitimate' : 'uncertain'
  return { case_id: selected.case_id, case: { status, verdict, fraud_probability: assessment.probability, pattern: assessment.pattern, pattern_description: assessment.pattern === 'none' ? undefined : findings.join(' ') , affected_txn_ids: amounts.length ? [selected.flagged_txn_id, ...related.map((row) => Number(field(row, 'TransactionID'))).filter(Number.isFinite).slice(0, 12)] : [], first_suspicious_txn_id: selected.flagged_txn_id, connected_card_ids: [selected.card_id], connected_device_profiles: context.data.identity_records.map((row) => field(row, 'DeviceInfo', 'DeviceType', 'id_30', 'id_31')).filter(Boolean), exposure_usd: exposure, evidence: context.evidence, similar_prior_cases: history.slice(0, 8).map((item) => ({ case_id: field(item, 'case_id'), outcome: field(item, 'outcome'), pattern: field(item, 'pattern'), relevance: 'Matched source customer/card/transaction context' })), summary: findings.join(' '), written_to_graph: writeResult.written, graph_case_id: writeResult.written && 'graph_case_id' in writeResult ? writeResult.graph_case_id ?? null : null }, evidence_requests: context.requests, next_best_actions: { initial, final, what_changed: 'Initial triage was reassessed after transaction, identity, historical, and TigerGraph tool results.' }, sar: { file: sarFile, reason: sarFile ? 'Strong evidence or exposure meets report criteria.' : '', narrative: sarFile ? `The investigation concerns customer ${selected.customer_id} and card ${selected.card_id}. Transaction ${selected.flagged_txn_id} was reviewed from the supplied transaction dataset. The activity occurred in the available transaction window. The observed channel and amount were taken from source records. Related transaction and identity evidence was considered where available. The report recommendation is based on the accumulated evidence and policy criteria.` : '', subjects: sarFile ? [selected.customer_id, selected.card_id] : [], total_amount_usd: sarFile ? exposure : 0, activity_dates: sarFile ? [selected.opened_at] : [] }, stop_reason: stopReason, tool_calls: context.tools, tokens: 0, latency_s: 0, system_status: { tigergraph: graphStatus.status, message: graphStatus.message } }
}
